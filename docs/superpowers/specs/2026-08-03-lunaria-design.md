...
핵심 가치는 모델을 단순 교체하는 데 있지 않다. Sol이 목표와 의사결정권을
보유한 채, 세부 코드 탐색과 실행을 독립 컨텍스트로 분리하여 주 스레드의
맥락 오염을 줄이고 안전하게 병렬화하는 운영 방식에 있다.
...
- 어떤 작업을 Luna Max에 맡기고 어떤 작업을 Sol에 남길지 빠르게 판단할 수
  있는 사용 가이드와 예시를 제공한다.
- GitHub 저장소에서 `npx skills`로 `lunaria`를 설치할 수 있게 한다.

## 목표가 아닌 것

- Sol을 주 에이전트에서 대체하는 것.
- 모든 구현을 작업 크기와 결합도에 관계없이 Luna에 강제하는 것.
- Luna가 제품 요구사항이나 여러 영역에 영향을 주는 아키텍처를 결정하게 하는 것.
- 별도의 Sol 리뷰어 역할이나 Terra 구현 레인을 설치하는 것.
- Codex Marketplace 플러그인과 Skills CLI 배포를 동시에 유지하는 것.
- Codex 내부 rollout 또는 세션 파일을 읽어 라우팅을 추론하는 것.
...
저장소 루트가 곧 배포 가능한 단일 스킬이다. 별도 Node 패키지나
`package.json`을 추가하지 않고 Skills CLI의 루트 `SKILL.md` 탐색 규약을
사용한다.
...
위 조건을 충족하는 서브 에이전트 작업에는 다른 일반 역할보다
`luna_worker`를 우선한다. 단, 작업을 설명하고 결과를 다시 검증하는 비용이
Sol이 직접 처리하는 비용보다 큰 작은 작업은 위임하지 않는다.

### 적합한 사용 사례

- 특정 모듈이나 파일 집합에 한정된 코드 리뷰
- 한 컴포넌트의 동작, 데이터 흐름, 결함 가능성 분석
- 다른 작업과 쓰기 경로가 겹치지 않는 독립 기능 구현
- 실패한 테스트의 원인 조사 또는 경계가 명확한 테스트 보강
- 서로 의존하지 않는 문서, 코드, 로그의 읽기 전용 병렬 조사

### 부적합한 사용 사례

- 사용자의 요구사항 또는 성공 조건을 먼저 결정해야 하는 작업
- 여러 모듈의 책임과 인터페이스를 바꾸는 아키텍처 결정
- 동일 파일, lockfile, 생성물, 마이그레이션을 공유하는 동시 쓰기
- 배포, 데이터 변경, 승인 같은 외부 부수 효과의 최종 판단
- 패킷 작성과 검증보다 Sol의 직접 처리가 더 짧은 단일 단계 작업

### 대표 운영 패턴

1. **읽기 전용 fan-out:** Sol이 독립적인 리뷰·분석 패킷을 여러 Luna에
   병렬로 보내고, 결과의 근거만 회수하여 판단한다.
2. **분리된 구현:** Sol이 아키텍처와 인터페이스를 확정한 뒤, 독점 경로를
   가진 구현을 Luna에 맡기고 직접 diff와 테스트를 검증한다.
3. **직렬 위임:** 같은 파일이나 공유 상태를 만지는 작업은 Luna를 사용하더라도
   하나씩 실행하고 각 결과를 Sol이 승인한 후 다음 패킷을 보낸다.

빠른 판단 질문은 다음과 같다. "Sol의 목표와 결정권을 유지한 채, 이 작업을
독립 컨텍스트에서 완료하고 증거로 검증할 수 있는가?" 아니면 Sol이 직접
처리한다.
...
```text
목표(Objective):
파일과 소유권(Files and ownership):
보존할 인터페이스(Interfaces):
허용 범위(Allowed scope):
...
Luna가 코드베이스의 유일한 작업자가 아니며, 다른 작업자나 사용자의 변경을
되돌리지 않고 현재 상태에 맞춰야 한다는 점을 명시한다.
...
모든 워커 보고는 검증 전까지 주장으로 취급한다. Sol은 실제 파일과 전체 diff를
확인하고, 범위 이탈 여부를 검사하며, 패킷에 명시한 검증을 직접 다시 실행한다.
...
Luna에 전달하는 입력은 패킷 수행에 필요한 최소 범위로 제한한다. 원시 탐색
로그와 장황한 사고 과정은 Luna 스레드에 남기고, Sol은 결정에 필요한 요약,
변경 파일, 검증 결과, 미해결 위험만 회수한다.

의존성이 있는 작업을 미리 배정하지 않고 단계별로 처리한다. 다른 워커의 결과가
필요한 작업은 해당 결과가 승인된 후에만 생성한다.

워커가 실패했거나 잘못된 결과를 냈다면 같은 패킷을 그대로 반복하지 않는다.
Sol이 실패 근거를 확인하고 목표, 범위, 인터페이스, 검증 중 필요한 항목을
수정한 뒤 새로운 패킷으로 다시 위임한다.

native spawn 또는 details metadata가 실제 역할, 모델, 추론 강도를 제공하면
`luna_worker`, `gpt-5.6-luna`, `max`와 대조한다. 공개 metadata가 일부 값을
제공하지 않으면 정적 템플릿 검사와 역할 노출 결과까지만 확인됐다고 명시하며,
관측하지 못한 값을 추정하거나 다른 역할의 성공으로 대체하지 않는다. Codex
내부 rollout 또는 세션 파일은 읽지 않는다.

## 설정과 구성 안전성

### Skills CLI 설치

GitHub에 배포된 뒤 사용자는 다음 명령으로 Codex 전역 스킬을 설치할 수 있다.

```bash
npx skills add SWARVY/Lunaria --skill lunaria --agent codex -g -y
```

이 명령은 스킬 패키지와 포함 리소스만 설치한다. 설치 과정에서
`~/.codex/agents/luna-worker.toml` 또는 `~/.codex/config.toml`을 자동으로
변경하지 않는다. 스킬을 처음 사용할 때 아래 관리 도구가 누락 상태를 감지하고,
diff를 제시한 뒤 명시적 승인을 받아 `luna_worker`만 설치한다.
...
- `npx skills add . --list`가 루트에서 `lunaria` 하나를 발견하는지 확인한다.
- 임시 디렉터리에서 `npx skills add <저장소 경로> --skill lunaria
  --agent codex --copy -y`를 실행하고 설치된 파일 구성을 검증한다.
...
- Skills CLI가 저장소를 단일 `lunaria` 패키지로 발견하고 격리된 Codex 대상에
  전체 리소스를 설치할 수 있다.
- 설정 도구가 `luna_worker` 설치를 안전하게 계획, 실행, 검증할 수 있다.
- Sol이 적합·부적합 사례와 위임 비용을 기준으로 Luna 사용 여부를 판단한다.
