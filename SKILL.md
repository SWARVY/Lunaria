...
Luna must not run any Git operation that mutates the working tree, index, refs,
branches, tags, stash, or worktrees, including but not limited to `git commit`,
`git add`, `git reset`, `git merge`, `git rebase`, `git stash`, `git clean`,
`git cherry-pick`, `git revert`, `git tag`, `git switch`, `git checkout`,
`git push`, or `git worktree` changes. Luna는 다른 에이전트를 생성할 수 없다.
